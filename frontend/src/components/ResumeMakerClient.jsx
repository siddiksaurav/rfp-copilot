...
export default function ResumeMakerClient({torName}) {
  ...
  const downloadAsPdf = () => {
    ...
    // Process the array - here are different approaches depending on your data structure:

    // Option 1: If cvs is an array of strings
    ...
    //     }
    //   }
    //   parseMarkdownToPDF(item);
    // });

    // Option 2: If cvs is an array of objects with a text/content property
    // cvs.forEach((item, index) => {
    //   if (index > 0) {
    //     yPosition += lineHeight * 2;
    //     if (yPosition > pageHeight - 20) {
    ...
    //     }
    //   }
    //   parseMarkdownToPDF(item.content || item.text || item.schedule);
    // });

    // Option 3: If you want to add a separator/title for each item
    // cvs.forEach((item, index) => {
    //   if (index > 0) {
    //     yPosition += lineHeight * 2;
    //     if (yPosition > pageHeight - 20) {
    ...
    //   doc.setFontSize(16);
    //   doc.setFont(undefined, 'bold');
    //   doc.text(`Schedule ${index + 1}`, 10, yPosition);
    //   yPosition += lineHeight + 5;
    //
    //   parseMarkdownToPDF(item);
    // });

    // Option 4: If you want each item on a new page
    cvs.forEach((item, index) => {
      if (index > 0) {
        doc.addPage();
        yPosition = 20;
    ...
    doc.save(`resume-${torName || 'default'}-${new Date().toISOString().split('T')[0]}.pdf`);
  };
              ...
              >
                <div className="flex items-center justify-between bg-white rounded-lg shadow-sm p-4 border">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button onClick={downloadAsPdf} className="ml-4">
